...
# 安全的文件写入
def safe_write_file(filepath, content, mode="wb"):
    """安全地写入文件，避免IO错误"""
            ...
            for file_name in img_files:
                if file_name.endswith(".jpg"):
                    try:
                        ...
                        print(f"已删除文件: img/{file_name}")
                    except Exception as e:
                        print(f"删除文件 img/{file_name} 时出错: {e}")
    except Exception as e:
        print(f"清理img目录时出错: {e}")
...
def all_evaluate(opts=None):
    ...
    try:
        ...
        try:
            ...
            opts["logger"].error(f"记录请求头详情时出错: {str(e)}")
        ...
        try:
            ...
            if req.status_code != 200:
                ...
                req = requests.get(url, headers=headers2, timeout=15)
            ...
            if "login.aspx" in req.url or "passport.jd.com" in req.url:
                ...
                return Comment_num  # 返回初始化的空字典
        
            # 保存响应内容以便调试
            try:
                safe_write_file("debug_response.html", req.content)
                opts["logger"].debug("已保存响应内容到debug_response.html文件")
            except Exception as e:
                opts["logger"].warning(f"保存响应内容失败: {str(e)}")
        except requests.exceptions.Timeout:
            opts["logger"].error("请求超时，可能是网络问题")
            return Comment_num
        except Exception as e:
            opts["logger"].error(f"请求页面时出错: {str(e)}")
            return Comment_num
    
        req_et = etree.HTML(req.text)
        
        # 尝试查找页面标题，确认已正确登录
        title = req_et.xpath('//title/text()')
        if title:
            opts["logger"].debug(f"页面标题: {title[0]}")
            
        # 检查是否有提示需要登录的信息
        login_notice = req_et.xpath('//div[contains(@class, "login-notice")]/text()')
        if login_notice:
        ...
        if not evaluate_data:
            # 尝试其他可能的XPath
            evaluate_data = req_et.xpath('//ul[contains(@class, "tabs-list")]/li')
            
        # 如果找到了评价标签列表
        if evaluate_data:
            opts["logger"].debug(f"找到 {len(evaluate_data)} 个评价标签")
            for i, ev in enumerate(evaluate_data):
                try:
                    # 提取标签名称
                    name_elements = ev.xpath("a/text() | span/text()")
                    if not name_elements:
                        continue
                        
                    na = name_elements[0].strip()
                    opts["logger"].debug(f"评价类型: {na}")
                    ...
                    try:
                        ...
                        if sup_elements:
                            num = sup_elements[0]
                            opts["logger"].debug(f"从b[@class='sup']标签中获取到数量: {num}")
                        else:
                            # 然后尝试获取普通的 <b> 标签
                            b_elements = ev.xpath("b/text()")
                            if b_elements:
                                num = b_elements[0]
                                opts["logger"].debug(f"从b标签中获取到数量: {num}")
                            else:
                                # 最后尝试其他可能包含数量的标签
                                num_elements = ev.xpath(".//span[contains(@class, 'num')]/text() | .//em[contains(@class, 'num')]/text()")
                                if num_elements:
                                    num = num_elements[0]
                                    opts["logger"].debug(f"从其他标签中获取到数量: {num}")
                                else:
                                    opts["logger"].debug(f"未找到'{na}'的数量元素，设为0")
                                    num = 0
                        ...
                        opts["logger"].error(f"提取数量时出错: {str(e)}")
                        num = 0
                        
                    # 根据标签名称设置对应的值
                    if "待评价" in na:
                        Comment_num["待评价订单"] = num
                    elif "待追评" in na:
                        Comment_num["待追评"] = num
                    elif "服务评价" in na:
                        Comment_num["服务评价"] = num
                    elif "晒单" in na:
                        Comment_num["晒单评价"] = num
                    elif "已评价" in na:
                        Comment_num["已评价"] = num
                except Exception as e:
                    opts["logger"].error(f"处理评价标签时出错: {str(e)}")
        
        # 第2种方法：如果从主页无法获取数据，尝试逐个访问各标签页面
        if Comment_num["待评价订单"] == 0 and Comment_num["待追评"] == 0 and Comment_num["服务评价"] == 0:
            opts["logger"].debug("从主页未获取到评价数据，尝试单独访问各标签页")
            ...
            for index, tab in enumerate(mycomment_tabs):
                # 跳过晒单(index=3)
                if index == 3:
                    continue
                ...
                tab_url = f"https://club.jd.com/myJdcomments/myJdcomment.action?tabType={tab}"
                opts["logger"].debug(f"访问标签页: {tab_url}")
                
                try:
                    # 发起请求并保存响应
                    tab_resp = requests.get(tab_url, headers=headers)
                    if tab_resp.status_code != 200:
                        opts["logger"].warning(f"标签页请求失败: {tab_resp.status_code}")
                        continue
                        
                    with open(f"debug_tab_{index}.html", "wb") as f:
                        f.write(tab_resp.content)
                        
                    et = etree.HTML(tab_resp.text)
                    
                    # 尝试多种方式获取数量
                    # 1. 查找页面中的数量信息
                    count_elements = et.xpath('//div[contains(@class, "number")]/text() | //div[contains(@class, "number")]/strong/text() | //span[contains(@class, "number")]/text()')
                    if count_elements:
                        for text in count_elements:
                            import re
                            # 尝试提取"X单/Y单"格式或"X单"格式
                            match = re.search(r'(\d+)(?:\s*单)?(?:/|／)?', text)
                            if match:
                                count = int(match.group(1))
                                Comment_num[tab_mapping[index]] = count
                                opts["logger"].debug(f"从标签页文本中获取到{tab_mapping[index]}数量: {count}")
                                break
                    
                    # 2. 如果没找到数量信息，尝试计算实际订单数量
                    if Comment_num[tab_mapping[index]] == 0:
                        # 尝试查找订单表格
                        order_tables = et.xpath('//table[contains(@class, "order-table")]/tbody')
                        if order_tables:
                            Comment_num[tab_mapping[index]] = len(order_tables)
                            opts["logger"].debug(f"通过订单表格计数获得{tab_mapping[index]}数量: {len(order_tables)}")
                        else:
                            # 尝试其他可能的元素
                            order_items = et.xpath('//div[contains(@class, "item") or contains(@class, "comment-item")]')
                            if order_items:
                                Comment_num[tab_mapping[index]] = len(order_items)
                                opts["logger"].debug(f"通过item元素计数获得{tab_mapping[index]}数量: {len(order_items)}")
                except Exception as e:
                    opts["logger"].error(f"处理{tab_mapping[index]}标签页时出错: {str(e)}")
...
def No(opts=None):
    ...
    # 如果N为空字典，说明发生了错误
    if not N or len(N) == 0:
        opts["logger"].error("获取评价数据失败，可能是Cookie已过期")
        return None
...
def main(opts=None):
    ...
    if "config" in opts and "cookie" in opts["config"]:
        ...
        try:
            ...
            opts["logger"].error(f"读取配置文件失败: {str(e)}")
            return
    ...
    # 更新jdspider中的cookie
    jdspider.cookie = ck if isinstance(ck, bytes) else ck.encode('utf-8')
    
    # 首先尝试访问登录页面，确认是否登录成功
    try:
        opts["logger"].debug("首先尝试访问京东登录状态验证页面...")
        session = requests.Session()
        login_check = session.get('https://order.jd.com/center/list.action', headers=headers, allow_redirects=False, timeout=15)
        ...
        opts["logger"].error("验证登录状态超时，请检查网络连接")
        return
    except Exception as e:
    ...
    # 检查是否需要停止或是否获取到评价数据
    if stop_flag or not N:
        opts["logger"].info("检测到停止标志或未获取到评价数据，正在终止评价...")
        return
        
    # 开始各类评价流程
    # 待评价
    if N["待评价订单"] != 0:
        opts["logger"].info(f"开始处理 {N['待评价订单']} 个待评价订单")
        ordinary(N, opts)
        opts["logger"].info("待评价订单处理完成")
    
    # 追评
    if N["待追评"] != 0 and not stop_flag:
        opts["logger"].info(f"开始处理 {N['待追评']} 个待追评订单")
        review(N, opts)
        opts["logger"].info("待追评订单处理完成")
    
    # 服务评价
    if N["服务评价"] != 0 and not stop_flag:
        opts["logger"].info(f"开始处理 {N['服务评价']} 个服务评价")
        Service_rating(N, opts)
        opts["logger"].info("服务评价处理完成")
    
    # 评价完成后清理临时图片文件
    try:
        delete_jpg()
    except Exception as e:
        opts["logger"].warning(f"清理临时图片文件时出错: {str(e)}")
    
    # 再次获取评价数量，检查是否全部完成
    if not stop_flag:
        N_after = all_evaluate(opts)
        if N_after:
            s_after = "----".join(["{} {}".format(i, N_after[i]) for i in N_after])
            opts["logger"].info(f"评价完成后状态: {s_after}")
    
    opts["logger"].info("所有评价流程完成！")
